// A modified version of the game from "https://chromedino.com/" with the autoRun added.

const AUTORUN_CONFIG = {  // CHANGE: The autorun config is created.
    DEFAULT_AUTO_RUN: true,  // Starts or not in the autoRun mode.
    SWITCH_KEY: "]",  // The key that switches the autoRun mode.
    C_B_VISUALIZATION: false,  // Visualize or not collision boxes.
...
(function() {
    ...
    function Runner(outerContainerId, opt_config) {
        ...
        this.autoRun = AUTORUN_CONFIG.DEFAULT_AUTO_RUN;  // CHANGE: The auto run works only when this one is True.
    ...
    Runner.config = {
        ...
        SPEED: 6,  // CHANGE: Originally, it was 6.
    ...
    Runner.prototype = {
        ...
        init: function() {
            ...
            window.addEventListener("keydown", (e) => {  // CHANGE: Makes the autoRun to switch on the target key's press.
                if (e.key == AUTORUN_CONFIG.SWITCH_KEY) {
        ...
        update: function() {
            ...
            if (this.activated) {
                ...
                var collision = hasObstacles && checkForCollision(this.horizon.obstacles[0], this.tRex, this.canvas.getContext('2d'), this);  // Now it takes the runner itself as an argument.
                if (!collision) {
    ...
    function checkForCollision(obstacle, tRex, opt_canvasCtx, runner) {
        ...
        if (boxCompare(tRexBox, obstacleBox)) {
            if (
                runner.autoRun &&  // CHANGE: It does automatic jumps only if the runner.autoRun.
                !tRex.jumping && !tRex.ducking && // CHANGE: It tries to jump if enlarged collision boxes collide.
                (
                    runner.distanceMeter.getActualDistance(runner.distanceRan) < AUTORUN_CONFIG.THRESHOLD && // CHANGE: The dinosaur has a higher chance to fail a jump when the score is greater than the threshold.
                    getRandomNum(0, 100) > AUTORUN_CONFIG.B_T_CHANCE ||  // CHANGE: It has a chance to not jump when a collision is detected. This one works only when the score is less than the threshold.
                    getRandomNum(0, 100) > AUTORUN_CONFIG.A_T_CHANCE  // CHANGE: It has a higher chance to fail a jump when the distance threshold is met.
                )
            ) {
